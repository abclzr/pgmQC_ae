...
int main(int argc, char* argv[])
{
   ...
   std::string metric_filename = directory_path + "/cuquantum_metric.txt";
   ...
   **********************/
   ...
   // Write the metric to metric_filename
	{
		std::ofstream metric_file(metric_filename);
		metric_file << "Memory usage on cpu: " << size_total << " bytes" << std::endl;
		metric_file << "Sparse tensor contraction time taken: " << minTimeCUTENSORNET * 1000 << " ms" << std::endl;
		metric_file << "Total flops: " << (int) flops << std::endl;
		metric_file.close();
	}
